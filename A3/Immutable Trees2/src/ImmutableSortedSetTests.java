...
public class ImmutableSortedSetTests {
    ...
    /**
     * Tests that the get at index operation works with a non-empty tree and values
     * in the tree including a null value.
     */
    @Test
    public void getAtIndexComplexTree() {
        ImmutableSortedSet tree = makeTree().add(null).add("Alice").add("Bob").add("Fred").add("George");

        assertEquals(null, tree.getAtIndex(0));
        assertEquals("Alice", tree.getAtIndex(1));
        assertEquals("Bob", tree.getAtIndex(2));
        assertEquals("Fred", tree.getAtIndex(3));
        assertEquals("George", tree.getAtIndex(4));
    }
     ...
     */
    @Test
    public void getIndexComplexTree() {
        ImmutableSortedSet tree = makeTree().add(null).add("Alice").add("Bob").add("Fred").add("George");

        assertEquals(1, tree.getIndex("Alice"));
        assertEquals(2, tree.getIndex("Bob"));
        assertEquals(3, tree.getIndex("Fred"));
        assertEquals(4, tree.getIndex("George"));
        assertEquals(0, tree.getIndex(null));
    }
     ...
     */
    @Test
    public void containsComplexTree() {
        ImmutableSortedSet tree = makeTree().add(null).add("Alice").add("Bob").add("Fred").add("George");
    ...
    /**
     * Tests that the remove operation works on a node with two childred (without
     * breaking the tree).
     */
    @Test
     ...
     */
    @Test
    public void keysComplexTree() {
        ImmutableSortedSet tree = makeTree().add("Alice").add("Bob").add("Fred").add("George").add(null);

        assertArrayEquals(new String[] { null, "Alice", "Bob", "Fred", "George" }, tree.keys());
        // ensure it works twice
        assertArrayEquals(new String[] { null, "Alice", "Bob", "Fred", "George" }, tree.keys());
    }
